      ...
      return deviceRegistry
        ...
        .map(homeAssistantDevice => {
          ...
          for (const entity of Object.values(homeAssistantDevice.entities)) {
            ...
            // https://github.com/home-assistant/core/tree/dev/homeassistant/components/light
            if (entityId.startsWith('light.')) {
              homeyDevice.class = homeyDevice.class || 'light';

              // Is this device a Light that can turn on/off?
              if (typeof entity.instance.state === 'string' || entity.instance.attributes.supported_color_modes.inludes('onoff')) {
              ...
              // !entity.instance.attributes.supported_color_modes.includes('onoff') is a failsafe, 
              // because the onoff capability is the basic capability for a light and if the light is natively not dimmable, it shouldnt go into this function
              if (entity.instance.attributes && entity.instance.attributes.supported_color_modes && !entity.instance.attributes.supported_color_modes.includes('onoff')) {
                ...
                // Is this device a light that can ONLY support brightness? 
                if (entity.instance.attributes.supported_color_modes.includes('brightness')) {
                  // failsafe: 
                  return homeyDevice;
                  // push dim capabilities, nothing else. 
                }

                // Is this device a Light that can change color temperature?         
                if (entity.instance.attributes.supported_color_modes.includes('color_temp')) {
                ...
                if (entity.instance.attributes.supported_color_modes.includes('hs')) {
                  // push light_saturation and light_hue
