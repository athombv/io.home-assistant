      ...
      return deviceRegistry
        ...
        .map(homeAssistantDevice => {
          ...
          for (const entity of Object.values(homeAssistantDevice.entities)) {
            ...
            if (friendlyName && friendlyName.length > 0 && homeyDevice.name.length > 0 && friendlyName.startsWith(homeyDevice.name)) {
            ...
            if (entityId.startsWith('light.')) {
              ...
              homeyDevice.iconOverride = (homeyDevice.iconOverride && homeyDevice.class !== 'sensor') ? homeyDevice.iconOverride : 'light-bulb';
            ...
            if (entityId.startsWith('switch.')) {
              ...
              homeyDevice.iconOverride = (homeyDevice.iconOverride && homeyDevice.class !== 'sensor') ? homeyDevice.iconOverride : 'plug';
              ...
              const capabilityId = (currentOnOffCapabilities.length > 0) ? `onoff.${currentOnOffCapabilities.length}` : 'onoff';
              ...
              homeyDevice.capabilitiesOptions[capabilityId].title = friendlyName || entity.instance.attributes['device_class'] || entityIdWithoutSwitch;
            ...
            if (entityId.startsWith('fan.')) {
              ...
              homeyDevice.iconOverride = (homeyDevice.iconOverride && homeyDevice.class !== 'sensor') ? homeyDevice.iconOverride : 'fan';
              ...
              if (entity.instance.attributes) {
                ...
                for (const [key, value] of Object.entries(HomeAssistantConstants.ENTITY_FAN_SUPPORTED_FEATURES)) {
                  ...
                  if (supportedFeatures & key) {
                    value.forEach(capabilityId => {
                      if (capabilityId === 'fan_mode') {
                        if (entity.instance.attributes['preset_modes'].every(mode => ['nature', 'normal'].includes(mode?.toLowerCase()))) {
                        ...
                        } else if (entity.instance.attributes['preset_modes'].every(mode => ['fan', 'auto', 'silent', 'favorite'].includes(mode?.toLowerCase()))) {
                          // Check if fan preset mode contains Fan, Auto, Silent and Favorite.
                          homeyDevice.capabilities.push('aircleaner_mode');
                          homeyDevice.capabilitiesOptions['aircleaner_mode'] = homeyDevice.capabilitiesOptions['aircleaner_mode'] || {};
                        ...
                        if (capabilityId === 'fan_speed') {
                          ...
                          homeyDevice.capabilitiesOptions[capabilityId].step = Math.floor(entity.instance.attributes['percentage_step']) / 100; // A bit of a hack because of rounding errors.
                        }
                      }
                    });
                  }
            ...
            if (entityId.startsWith('sensor.')) {
              ...
              if (entity.instance.attributes) {
                ...
                if (capabilityId) {
                  ...
                  homeyDevice.capabilitiesOptions['measure_wind_strength'] = homeyDevice.capabilitiesOptions['measure_wind_strength'] || {};
                  ...
                  homeyDevice.capabilities.push(capabilityId);
                  homeyDevice.capabilitiesOptions[capabilityId] = homeyDevice.capabilitiesOptions[capabilityId] || {};
                  homeyDevice.capabilitiesOptions[capabilityId].title = friendlyName || entity.instance.attributes['device_class'] || entityIdWithoutSensor;
            ...
            if (entityId.startsWith('binary_sensor.')) {
              ...
              if (entity.instance.attributes && entity.instance.attributes['device_class'] !== 'update') {
                ...
                if (capabilityId === 'smoke' || capabilityId === 'heat') homeyDevice.iconOverride = 'smoke-detector';

                if (capabilityId) {
                  homeyDevice.capabilities.push(capabilityId);
                  homeyDevice.capabilitiesOptions[capabilityId] = homeyDevice.capabilitiesOptions[capabilityId] || {};
                  homeyDevice.capabilitiesOptions[capabilityId].entityId = entityId;
                } else if (entityId.endsWith('_charging_status')) {
                  homeyDevice.capabilities.push('alarm_charging');
                  homeyDevice.capabilitiesOptions['alarm_charging'] = homeyDevice.capabilitiesOptions['alarm_charging'] || {};
                  homeyDevice.capabilitiesOptions['alarm_charging'].entityId = entityId;
                } else {
            ...
            if (entityId.startsWith('cover.')) {
              const coveringType = entity.instance.attributes['device_class']
                ? HomeAssistantConstants.ENTITY_COVER_CLASS_MAP[entity.instance.attributes['device_class']]
                : 'windowcoverings';
              ...
              if (entity.instance.attributes) {
                ...
                for (const [key, value] of Object.entries(HomeAssistantConstants.ENTITY_COVER_SUPPORTED_FEATURES)) {
                  // Check if the key is part of the supported features binary value.
                  if (supportedFeatures & key) {
                    value.forEach(capabilityId => {
                      homeyDevice.capabilities.push(capabilityId);
