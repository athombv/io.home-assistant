...
const PHONE_CAPABILITIES = {
    'sensor.gm1913_battery_temperature': 'measure_temperature',
    'sensor.gm1913_battery_level': 'measure_battery'
};
    ...
    homeyMapper(entities) {
        if (this.oldEntities.length !== entities.length) { // !==
            ...
            for (let i = 0; i < mappedDevices.length; i++) {
                ...
                } if(i >= 4 && mappedDevices[i].name === 'GM1913 Battery Temperature' && mappedDevices[i-1].name === 'GM1913 Battery Level'){
                    mappedDevices.push({
                        name: "GM1913",
                        data:{
                            id: "sensor.gm1913",
            ...
            console.log(JSON.stringify(mappedDevices, null, 4));
