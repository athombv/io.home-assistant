...
module.exports = class HAUtil {
  ...
  static LIGHT_CAPABILITIES = ['onoff', 'dim', 'light_temperature'];
  ...
  static getCapabilityFromEntity(entity) {
    ...
    if (entity.attributes && entity.attributes.supported_color_modes && entity.attributes.supported_features) { // 99% chance the entity that is being scanned is a light
      ...
      if (entity.attributes.supported_color_modes.includes("hs") && entity.attributes.supported_color_modes.includes("color_temp")) { // you change the supported color modes through the state_changed event type
        this.LIGHT_CAPABILITIES.push("light_hue");
        this.LIGHT_CAPABILITIES.push("light_saturation");
      }
      return this.LIGHT_CAPABILITIES;
    }
