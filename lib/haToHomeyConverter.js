...
class Converter {
    ...
    stateChange(event) {
        try {
            ...
            const { data } = event; // you unpack the event parameter into the const data. Event consists in this case of the data : {entity_id, stateUpdate}
            if (data) {     // if the 'data' condition is true. the condition is false when:
                // false, 0, NaN, undefined, null, "" [empty string]
                // so, when entity_id is not null and when stateUpdate is also not null or empty, this code gets executed in the update
                const entityId = data.entity_id;
                console.log('device:', deviceIds);
                deviceIds.forEach(deviceId => {
                    let i = 0;
                    if (deviceId === 'sensor.lumi_lumi_weather' && i < 1) {
                        console.log('device ID detected');
                        //deviceIds = deviceIds + (WEATHER_CAPABILITIES[key]);
                        for (const key of Object.keys(WEATHER_CAPABILITIES)) {
                            deviceIds.push(key);
                            //this.registerDevice(entities[key], this)
                        }
                        i++;
                    }
                    ...
                    //     }
                    // }
                    //return this.deviceSubIds;
                });
                console.log('new Devices:', deviceIds);
                deviceIds.forEach(deviceId => {
                    const device = this._devices[deviceId];
                    if(device !== null) {
                        if(deviceId === entityId) {
            ...
            for (let i = 0; i < mappedDevices.length; i++) {
                ...
                if (i >= 4 && mappedDevices[i].name === 'LUMI weather sensor temperature' && mappedDevices[i - 1].name === 'LUMI weather sensor pressure' && mappedDevices[i - 2].name === 'LUMI weather sensor power' && mappedDevices[i - 3].name === 'LUMI weather sensor humidity') {
                    ...
                    mappedDevices.push({
                        ...
                        data: {
                            id: "sensor.lumi_lumi_weather_temperature", // changed from sensor.lumi_lumi_weather
